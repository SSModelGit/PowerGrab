...
model Test2DoubleBoneJointModel
  ...
  Modelica.Blocks.Sources.Pulse anteriorForwardMusclePulse(period=1.5)
    annotation (Placement(transformation(extent={{-188,0},{-168,20}})));
  Modelica.Blocks.Sources.Pulse posteriorForwardMusclePulse(period=1.5,
      startTime=0.75)
    annotation (Placement(transformation(extent={{-188,-40},{-168,-20}})));
equation
  ...
  connect(realBoneDoubleJoint.threadPointAnterior,
    anteriorUpperMuscleConnection.frame_a) annotation (Line(
      points={{0,10},{0,10},{0,40},{4,40},{4,39.6}},
      color={95,95,95},
      pattern=LinePattern.Dot));
  ...
  connect(posteriorUpperMuscleConnection.frame_b, realBoneDoubleJoint.attachPointPosterior)
    annotation (Line(
      ...
      color={95,95,95},
      thickness=0.5));
  connect(posteriorForwardMusclePulse.y, posteriorMuscle.forcePValue)
    annotation (Line(points={{-167,-30},{-144,-30},{-144,-22},{-56,-22},{-56,
          -66.5685},{-46.6807,-66.5685}}, color={0,0,127}));
  connect(anteriorForwardMusclePulse.y, anteriorMuscle.forcePValue) annotation
    (Line(points={{-167,10},{-164,10},{-164,92},{-52,92},{-52,68},{-52,70},{-52,
          69.4315},{-50.6807,69.4315}}, color={0,0,127}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio=false)));
end Test2DoubleBoneJointModel;
